<x-layout>
  ...
  <div class="md:flex justify-between">
    <div class="mb-4 md:w-1/2">
      ...
      @if(!$hasCommented)
      <a href="{{ route('user.comment', ['id' => $restaurant->id]) }}" class="border-b border-black">口コミを投稿する</a>
      @endif
    ...
    @if(count($errors) > 0)
    <p class="absolute top-10 left-1/2 -translate-x-1/2 text-red-500">予約に失敗しました</p>
    @endif
    <div x-data="{ date:'', time: '', num: '' }" class="bg-blue-600 rounded-md pt-4 md:w-5/12 h-fit">
      <h3 class="text-white text-lg font-bold ml-8">予約</h3>
      <form method="POST" action="{{ route('user.detail', ['id' => $restaurant->id]) }}">
        @csrf
        ...
        <div class="w-10/12 bg-blue-500 py-4 rounded-md ml-4 mt-4">
          <table class="border-separate border-spacing-x-8 font-medium text-gray-100">
            ...
            <tr>
              <td>Number</td>
              <td x-text="num" class="after:content-['\00A0人']"></td>
            </tr>
          </table>
        </div>
        <button class="w-full mt-16 py-2 text-lg bg-blue-700 text-white rounded-b-md">予約する</button>
      </form>
    </div>
  </div>
</x-layout>