<x-layout>
  <div class="md:flex justify-between">
    <div class="h-fit mt-8 md:mt-20 md:w-5/12">
      <h2 class="px-16 text-2xl md:text-2xl text-center font-medium pb-4 md:pb-10">口コミを編集しますか？</h2>
      <x-comment-card :comment="$comment" />
    </div>
    <form method="POST" enctype="multipart/form-data" action="{{ route('user.recomment', ['id' => $comment->id]) }}" class="md:w-1/2 mt-8 md:mt-0"> @csrf
      ...
      <div x-data="{ text: '{{ $comment->body }}', count: {{ mb_strlen($comment->body) }}}">
        <textarea x-model="text" x-on:input="count = text.length" class="w-full focus:ring-0 focus:ring-gray-300 resize-none bg-gray-200" name="body" cols="30" rows="5"></textarea>
        <p x-text="count" :class="{ 'text-red-500': count > 400 }" class="text-right text-sm -mt-1 font-medium after:content-['/400\00A0(最高文字数)']"></p>
      </div>
      <h4 class="text-xl font-medium pt-6 pb-2">画像の変更</h4>
