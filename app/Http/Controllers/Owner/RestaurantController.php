...
class RestaurantController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query->where('owner_id', Auth::guard('owners')->id());
    ...
    public function detail($id)
    {
        $restaurant = Restaurant::where('id', $id)
                                ->where('owner_id', Auth::guard('owners')->id())
                                ->first();
        if(is_null($restaurant)) {
            return view('warning', $this->getWarningData('店舗が見つかりません'));
        }

        return view('owner.detail', compact('restaurant'));
    }

    public function update(RestaurantRequest $request, $id)
    {
        $restaurant = Restaurant::find($id);
        if(is_null($restaurant) || $restaurant->owner_id != Auth::guard('owners')->id()) {
            return view('warning', $this->getWarningData('店舗が見つかりません'));
        }
    ...
    public function create(CreateRestaurantRequest $request)
    {
        ...
        return redirect(route('owner.detail', ['id' => $restaurant->id]));
    }
